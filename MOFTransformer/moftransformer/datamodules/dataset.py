...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        zeopp_label_path="",
        topo_label_path="",
        chem_label_names=None,
        zeopp_label_names=None,
        topo_label_names=None,
    ):
        """
        ...
        self.chem_label_path = chem_label_path
        self.zeopp_label_path = zeopp_label_path
        self.topo_label_path = topo_label_path
        self.chem_label_names = chem_label_names or []
        self.zeopp_label_names = zeopp_label_names or []
        self.topo_label_names = topo_label_names or []
        ...
        for task in tasks:
            if task in ["mtp", "vfp", "moc", "bbc"]:
                ...
                self.tasks[task] = self._load_descriptor_task(
                    task=task,
                    csv_path=self.chem_label_path,
                    feature_names=self.chem_label_names,
                )
            elif task == "zeopp":
                self.tasks[task] = self._load_descriptor_task(
                    task=task,
                    csv_path=self.zeopp_label_path,
                    feature_names=self.zeopp_label_names,
                )
            elif task == "topo":
                self.tasks[task] = self._load_descriptor_task(
                    task=task,
                    csv_path=self.topo_label_path,
                    feature_names=self.topo_label_names,
                )

    def __len__(self):
        return len(self.cif_ids)

    def _load_descriptor_task(self, task, csv_path, feature_names):
        if not csv_path:
            raise ValueError(f"task={task} 需要提供 csv_path")
        if not feature_names:
            raise ValueError(f"task={task} 需要至少一个 feature_names")
        if not os.path.isfile(csv_path):
            raise FileNotFoundError(f"{csv_path} doesn't exist")

        print(f"read {csv_path} for task={task}...")
        rows = {}
        with open(csv_path, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                cif_id = row["cif_id"]
                rows[cif_id] = [float(row[name]) for name in feature_names]

        missing = [cif_id for cif_id in self.cif_ids if cif_id not in rows]
        ...
        if missing:
            print(
                f"[warn] task={task} 有 {len(missing)} 个 cif_id 在标签表里缺失，使用均值填充，例如: {missing[:10]}"
            )

        values = []
        for cif_id in self.cif_ids:
            raw = np.array(rows.get(cif_id, means.tolist()), dtype=np.float32)
    ...
    def get_graph(self, cif_id):
        """
        读取 prepare_data 生成的 .graphdata 文件（新格式）：
        [cif_id, atom_num, nbr_idx, nbr_dist, uni_idx, uni_count, angle_src, angle_dst, angle_fea]
        """
        file_graph = os.path.join(self.data_dir, self.split, f"{cif_id}.graphdata")
        graphdata = pickle.load(open(file_graph, "rb"))

        # graphdata = ["cif_id", "atom_num", "nbr_idx", "nbr_dist",
        #              "uni_idx", "uni_count",
        #              "angle_src", "angle_dst", "angle_fea"]
    ...
    def collate(batch, img_size):
        ...
        Returns:
            dict_batch (dict): [cif_id, atom_num, nbr_idx, nbr_fea, crystal_atom_idx,
                                uni_idx, uni_count, angle_src, angle_dst, angle_fea,
                                grid, false_grid_data, target]
        """
        ...
        for i, nbr_idx in enumerate(batch_nbr_idx):
            ...
            # shift 邻接索引到 batch 全局节点索引
            nbr_idx += base_idx
            base_idx += n_i

            # 对角度图的边 id 做偏移（以 edge 为节点，id = row * M + col）
            if batch_angle_src is not None and batch_angle_src[i] is not None:
                e_i = n_i * m_i  # 本结构的 edge 数
        ...
        if "false_grid_data" in dict_batch.keys():
            ...
            for bi in range(batch_size):
                ...
                if batch_false_cell[bi] == [img_size, img_size, img_size]:
                    orig = orig[None, None, :, :, :]
                else:
                    orig = interpolate(
                        orig[None, None, :, :, :],
