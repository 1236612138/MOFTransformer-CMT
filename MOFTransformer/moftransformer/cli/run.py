...
class CLICommand:
    ...
    def add_arguments(parser):
        # Add arguments for gpus and distributed_backend
        parser.add_argument('--gpus', type=int, default=1, help='Number of GPUs to use for training (default: 1)')
        parser.add_argument('--distributed_backend', type=str, default='none', choices=['none', 'ddp', 'dp', 'horovod'], 
                            help='Backend for distributed training (default: none)')
        ...
        parser.add_argument('--downstream', "-d", type=str, default=None, help="Name of user-specific task.")
        parser.add_argument('--log_dir', "-l", default='./logs', type=str, help='Directory to save log, models, and params.')
        parser.add_argument('--test_only', "-t", action='store_true', help='Test only, without training the model.')
    ...
    def run(args):
        ...
        # 确保传递 gpus 和 distributed_backend
        gpus = args.gpus
        ...
        run(
            ...
            gpus=gpus,  # 添加 gpus 配置
            distributed_backend=distributed_backend,  # 添加 distributed_backend 配置
            **kwargs
        )
