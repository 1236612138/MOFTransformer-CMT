...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(
        ...
        stem_dim = max(embed_dim // 4, 32)
        mid_dim = max(embed_dim // 2, stem_dim)

        # 先用轻量 3D 卷积提取局部空间模式，再用 stride=patch_size 的卷积变成 token。
        self.stem = nn.Sequential(
            nn.Conv3d(in_chans, stem_dim, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm3d(stem_dim),
            nn.GELU(),
            nn.Conv3d(stem_dim, mid_dim, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def forward(self, x):
        x = self.stem(x)
        x = self.proj(x)  # [B, embed_dim, gh, gw, gd]
