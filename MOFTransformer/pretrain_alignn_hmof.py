...
def build_loss_names():
    ...
    chem_weight = env_weight("ALIGNN_PRETRAIN_CHEM_WEIGHT", "0")
    zeopp_weight = env_weight("ALIGNN_PRETRAIN_ZEOPP_WEIGHT", "0")
    topo_weight = env_weight("ALIGNN_PRETRAIN_TOPO_WEIGHT", "0")
    symmetry_weight = env_weight("ALIGNN_PRETRAIN_SYMMETRY_WEIGHT", "0")
...
def validate_pretrain_dataset(
    root_dataset: Path,
    loss_names: dict,
    chem_label_path: Optional[Path] = None,
    zeopp_label_path: Optional[Path] = None,
    topo_label_path: Optional[Path] = None,
):
    ...
    if loss_names.get("chem", 0) > 0 and chem_label_path is not None:
        required_files.append(chem_label_path)

    if loss_names.get("zeopp", 0) > 0 and zeopp_label_path is not None:
        required_files.append(zeopp_label_path)

    if loss_names.get("topo", 0) > 0 and topo_label_path is not None:
        required_files.append(topo_label_path)
...
if __name__ == "__main__":
    ...
    uncertainty_weighting = os.environ.get(
        "ALIGNN_PRETRAIN_UNCERTAINTY_WEIGHTING", "0"
    ).strip().lower() in {"1", "true", "yes", "on"}
    descriptor_head_type = os.environ.get(
        "ALIGNN_PRETRAIN_DESCRIPTOR_HEAD_TYPE", "linear"
    ).strip()
    chem_head_type = os.environ.get(
        "ALIGNN_PRETRAIN_CHEM_HEAD_TYPE", descriptor_head_type
    ).strip()
    zeopp_head_type = os.environ.get(
        "ALIGNN_PRETRAIN_ZEOPP_HEAD_TYPE", descriptor_head_type
    ).strip()
    symmetry_mode = os.environ.get(
        "ALIGNN_PRETRAIN_SYMMETRY_MODE", "grid_flip_rot"
    ).strip()
    symmetry_cls_weight = float(
        os.environ.get("ALIGNN_PRETRAIN_SYMMETRY_CLS_WEIGHT", "1.0")
    )
    symmetry_graph_weight = float(
        os.environ.get("ALIGNN_PRETRAIN_SYMMETRY_GRAPH_WEIGHT", "0.5")
    )
    symmetry_grid_weight = float(
        os.environ.get("ALIGNN_PRETRAIN_SYMMETRY_GRID_WEIGHT", "0.5")
    )
    symmetry_pred_weight = float(
        os.environ.get("ALIGNN_PRETRAIN_SYMMETRY_PRED_WEIGHT", "0.25")
    )
    ...
    task_weight_schedule = {}
    for task in ["ggm", "mpp", "chem", "zeopp"]:
        start_weight = env_optional_float(f"ALIGNN_PRETRAIN_{task.upper()}_START_WEIGHT")
        if start_weight is None:
            continue
        task_weight_schedule[task] = {
            "start": start_weight,
            "ramp_start_epoch": env_int(
                f"ALIGNN_PRETRAIN_{task.upper()}_RAMP_START_EPOCH", "0"
            ),
            "ramp_epochs": env_int(
                f"ALIGNN_PRETRAIN_{task.upper()}_RAMP_EPOCHS", "0"
            ),
        }
    ...
    chem_label_path = Path(
        os.environ.get(
            "ALIGNN_PRETRAIN_CHEM_LABEL_PATH",
            "/home/yihaoyu/docker/bag/MOFTransformer_CMT/generated_labels/hmof_1pct_chem/chem_labels.csv",
        )
    )
    zeopp_label_path = Path(
        os.environ.get(
            "ALIGNN_PRETRAIN_ZEOPP_LABEL_PATH",
            "/home/yihaoyu/docker/bag/MOFTransformer_CMT/generated_labels/hmof_1pct_zeopp/zeopp_labels.csv",
        )
    )
    topo_label_path = Path(
        os.environ.get(
            "ALIGNN_PRETRAIN_TOPO_LABEL_PATH",
            "/home/yihaoyu/docker/bag/MOFTransformer_CMT/generated_labels/hmof_1pct_topo/topo_labels.csv",
        )
    )
    ...
    chem_label_names = [
        ...
        for x in os.environ.get(
            "ALIGNN_PRETRAIN_CHEM_LABEL_NAMES",
            "density,metal_fraction,fraction_C,fraction_N,fraction_O",
        ).split(",")
        if x.strip()
    ]
    ...
    zeopp_label_names = [
        x.strip()
        for x in os.environ.get(
            "ALIGNN_PRETRAIN_ZEOPP_LABEL_NAMES",
            "pld,lcd,av_fraction,nav_fraction",
        ).split(",")
        if x.strip()
    ]
    topo_label_names = [
        x.strip()
        for x in os.environ.get(
            "ALIGNN_PRETRAIN_TOPO_LABEL_NAMES",
            "num_channels,num_pockets,has_accessible_channel,has_pocket",
        ).split(",")
        if x.strip()
    ]
    ...
    validate_pretrain_dataset(
        root_dataset,
        loss_names,
        chem_label_path=chem_label_path,
        zeopp_label_path=zeopp_label_path,
        topo_label_path=topo_label_path,
    )
    ...
    config.update(
        {
            ...
            "chem_head_type": chem_head_type,
            "zeopp_head_type": zeopp_head_type,
            "symmetry_mode": symmetry_mode,
            "symmetry_cls_weight": symmetry_cls_weight,
            "symmetry_graph_weight": symmetry_graph_weight,
            "symmetry_grid_weight": symmetry_grid_weight,
            "symmetry_pred_weight": symmetry_pred_weight,
            # 课程式任务升权配置。
            "task_weight_schedule": task_weight_schedule,
            # 图长度上限。
            ...
            "std": None,
            # 新增结构标签任务配置。
            "chem_label_path": str(chem_label_path),
            "zeopp_label_path": str(zeopp_label_path),
            "topo_label_path": str(topo_label_path),
            "chem_label_names": chem_label_names,
            "zeopp_label_names": zeopp_label_names,
            "topo_label_names": topo_label_names,
        }
    )
    ...
    for key in [
        ...
        "descriptor_head_type",
        "chem_head_type",
        "zeopp_head_type",
        "symmetry_mode",
        "symmetry_cls_weight",
        "symmetry_graph_weight",
        "symmetry_grid_weight",
        "symmetry_pred_weight",
        "task_weight_schedule",
        "num_workers",
        "load_path",
        "chem_label_path",
